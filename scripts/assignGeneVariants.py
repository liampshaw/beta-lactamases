...
def get_options():
    ...
    parser.add_argument('--outputprefix', help='output prefix', type=str, required=False, default='gene-variants')
    parser.add_argument('--inputprefix',help='prefix for input files', required=False, default='./')
...
def main():
	args = get_options()
	card_seqs = SeqIO.to_dict(SeqIO.parse('../data/nucleotide_fasta_protein_homolog_model.fasta', 'fasta'))

	gene = args.gene
	gene_family = re.sub("-.*", "", gene) # technically doesn't handled CTX-M properly, but ok

	fasta_file = args.inputprefix+'_focal_gene.fa'
	seqs_to_assign = SeqIO.to_dict(SeqIO.parse(fasta_file, 'fasta'))

	gene_seqs = {k:v for k, v in card_seqs.items() if gene_family in k}


	seqs_aa = {str(v.seq.translate()): re.sub('.*\\|', '', k)  for k, v in gene_seqs.items()}

	seqs_nt = {str(v.seq):re.sub('.*\\|', '', k) for k, v in gene_seqs.items()}



	#print(len(gene_seqs))
	#print(gene_seqs.items())
